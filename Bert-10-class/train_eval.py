...
def infer(config, model, data_iter, test=False):
    ...
    for i in range(labels_all.shape[0]):
        print('text:')
        print(alltext[i])
        print('true:'+clsnam[int(labels_all[i])])
        print('predict:'+clsnam[int(predict_all[i])])

def inferone10(config, model, text, test=False):
    with open('newsdataset/data/class.txt','r',encoding='utf-8') as f:
        clsnam = f.readlines()
    for i in range(len(clsnam)):
        clsnam[i] = clsnam[i].strip('\n')
    ...
    with torch.no_grad():
        ...
        predic = torch.max(outputs.data, 1)[1].cpu().numpy()
        predict_all = np.append(predict_all, predic)
    return clsnam[int(predict_all[0])]