...
class RequestApi(object):
    def __init__(self, appid, secret_key, upload_file_path, save_result_path):
        self.appid = appid
        ...
        if result["ok"] == 0:
            ...
            if apiname == api_get_result:
                ...
                for i in range(data_len):
                    json_dict2 = {}
                    json_dict2["Start_frameID"] = news_data[i]["bg"]
                    json_dict2["end_frameID"] = news_data[i]["ed"]
                    print(json_dict2["Start_frameID"],
                          json_dict2["end_frameID"])
...
def video_to_audio(video_file,audio_file):
    ...
    # sound=AudioSegment.from_wav(audio_file+".wav")
...
def data_process(result_path):
    ...
    f_res = open(result_path, "w", encoding='utf8')
    f_res.write(res)
            
    

def run(video_file,audio_file,id,key,result_path):
    audio_file = video_to_audio(video_file,audio_file)
    api = RequestApi(appid=id, secret_key=key,
                     upload_file_path=audio_file, save_result_path=result_path)
    api.all_api_request()
    data_process(result_path)
